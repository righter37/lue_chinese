...
def split_into_sentences(paragraph: str) -> list[str]:
    ...
    # 3. Split the text into sentences using English sentence-ending punctuation.
    # The lookbehind `(?<=[.!?])` keeps the delimiter with the sentence.
    sentences = re.split(r'(?<=[.!?])\s+', paragraph)

    # 4. Restore the periods, split on Chinese sentence-ending punctuation, and clean up.
    restored_sentences = []
    for sentence in sentences:
        if sentence:
            restored = sentence.replace(placeholder, ".")
            # Also split on Chinese sentence-ending punctuation, keeping the delimiter.
            chinese_splits = re.split(r'(?<=[。！？])', restored)
            for cs in chinese_splits:
                # Only include chunks that contain actual words or CJK characters.
                # This filters out orphaned punctuation like standalone "！" or "？".
                if cs.strip() and re.search(r'[\w\u4e00-\u9fff]', cs, re.UNICODE):
                    restored_sentences.append(cs)
...
def sanitize_text_for_tts(text):
    ...
    # Chinese punctuation (，。！？；：、…) is explicitly preserved for natural TTS pauses.
